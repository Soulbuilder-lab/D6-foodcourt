...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ─────────────────────────────────────────────────────────
  // RECEIPT MODAL INJECTION (UNCHANGED)
  // ─────────────────────────────────────────────────────────
  ...
  const renderHistory = async () => {
    ...
    try {
      ...
      const merged = [...tableOrders];
      allOrders.forEach(lo => {
        if (!merged.find(to => to.id === lo.id)) {
          merged.push(lo);
        }
      });
      ...
      historyList.innerHTML = `
        <div style="text-align:center; padding:30px; color:#e76f51;">
          ...
          <button onclick="location.reload()" style="padding:8px 16px; background:#154C63; color:#fff; border:none; border-radius:6px; cursor:pointer;">
            Retry
          </button>
        </div>`;
    }
