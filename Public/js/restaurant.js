...
// ✅ Read logged-in store FIRST (set by admin-login.js)
const loggedInStore = localStorage.getItem('loggedInStore') || 'all';

const STORAGE_KEY = 'foodcourt_orders';
const COMPLETED_KEY = `foodcourt_completed_${loggedInStore}`; // now defined after loggedInStore
const storeNames = {
  xiaoyun: 'Small Cloud',
...
function renderOrders() {
  ...
  ['new', 'preparing', 'ready'].forEach(status => {
    ...
    // ✅ FILTER by status AND store (exclude completed — they stay in storage for account.js)
...
function updateStatus(orderId, newStatus) {
  ...
  if (newStatus === 'completed') {
    completedCount++;
    // ✅ Do NOT splice — keep order in storage so account.js can still read it
