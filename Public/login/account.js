...
function setupStorageSync() {
    ...
    const storeNames = {
        xiaoyun: 'Small Cloud', goldenbowl: 'The Golden Bowl', zapfan: 'Zap Fan',
        bobamilk: 'Boba Milk', vitaminc: 'Vitamin C', all: 'All Stalls'
    };
...
function updateStats() {
    ...
    const totalProfit = ordersData.reduce((sum, order) => {
        return sum + (parseFloat(order.total) || parseFloat(order.price) || 0);
    }, 0);
...
function renderTable() {
    ...
    const sortedOrders = [...ordersData].sort((a, b) => 
        new Date(b.orderDate || b.date || 0) - new Date(a.orderDate || a.date || 0)
    );
    ...
    tbody.innerHTML = sortedOrders.map(order => {
        const time = new Date(order.orderDate || order.date || Date.now()).toLocaleTimeString('en-MY', { 
            hour: '2-digit', 
            minute: '2-digit' 
        });
        ...
        return `
            <tr>
                ...
                <td>${foodDisplay}</td>
                <td class="price-cell">RM ${isNaN(priceValue) ? '0.00' : priceValue.toFixed(2)}</td>
...
function refreshData() {
    ...
    setTimeout(() => {
        const allOrders = JSON.parse(localStorage.getItem('foodcourt_orders')) || 
                         JSON.parse(localStorage.getItem('orders')) || [];
        const loggedInStore = localStorage.getItem('loggedInStore') || 'all';
        const storeNames = { xiaoyun: 'Small Cloud', goldenbowl: 'The Golden Bowl', wokroll: 'Wok & Roll', boba: 'The Boba Shop', vitaminc: 'Vitamin C', all: 'All Stalls' };
        const currentStoreName = storeNames[loggedInStore] || 'All Stalls';
...
window.openOrderModal = function(orderId) {
    ...
    document.getElementById('modal-discount').textContent = parseFloat(order.discount || 0) > 0 ? `-RM ${formatMoney(order.discount)}` : 'RM 0.00';
    document.getElementById('modal-delivery').textContent = `RM ${formatMoney(order.delivery)}`;
    document.getElementById('modal-tax').textContent = `RM ${formatMoney(order.tax)}`;
...
`;
    ...
    receiptText += `
----------------------------------
Subtotal:       RM ${parseFloat(order.subtotal || 0).toFixed(2)}
Tax (6%):       RM ${parseFloat(order.tax || 0).toFixed(2)}
----------------------------------
TOTAL PAID:     RM ${parseFloat(order.total || 0).toFixed(2)}
==================================
