...
// 1. 初始化购物车数据
let cartItems = JSON.parse(localStorage.getItem('healthyCart')) || [];
    ...
    overlay.innerHTML = `
        <div style="background:#fff;border-radius:18px;padding:32px 28px;max-width:360px;width:90%;text-align:center;box-shadow:0 12px 40px rgba(0,0,0,0.2);animation:tpSlideUp 0.25s ease;">
            <div style="width:64px;height:64px;background:#E8F5E9;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px;">
                <ion-icon name="restaurant-outline" style="font-size:2rem;color:#4CAF50;"></ion-icon>
            </div>
            ...
            <input id="tp-input" type="text" placeholder="e.g. 5, A3, B12" maxlength="10"
                ...
                oninput="this.style.borderColor='#4CAF50'"
            ...
            <div style="display:flex;gap:10px;">
                ...
                <button id="tp-confirm"
                    style="flex:1;padding:11px;background:#4CAF50;color:#fff;border:none;border-radius:10px;cursor:pointer;font-size:0.9rem;font-weight:700;transition:background 0.2s;"
...
function addToCart(itemName, itemPrice) {
    ...
    // 保存到 localStorage
    localStorage.setItem('healthyCart', JSON.stringify(cartItems));
...
function filterMenu(category, event) {
    ...
    buttons.forEach(btn => {
        ...
        btn.style.color = 'var(--green14)';
        btn.style.borderColor = 'var(--green6)';
    });
    ...
    if (clickedButton) {
        clickedButton.style.backgroundColor = 'var(--green12)';
        clickedButton.style.color = 'var(--whitecolor)';
        clickedButton.style.borderColor = 'var(--green12)';
    }
