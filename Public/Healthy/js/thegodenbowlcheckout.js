...
document.addEventListener('DOMContentLoaded', () => {
    const cartData = JSON.parse(localStorage.getItem('goldenBowlCart')) || [];
    ...
    if (cartData.length === 0) {
        ...
        window.location.href = 'thegoldenbowlcart.html';
        return;
    }
    ...
    form.addEventListener('submit', (e) => {
        ...
        // ✅ 保存订单信息
        const orderData = {
            id: 'ORD-' + Date.now().toString().slice(-6),  // ← add this
            restaurant: 'The Golden Bowl',                   // ← add this (exact name!)
            timestamp: new Date().toISOString(),
            table: localStorage.getItem('tableNumber') || 'Kiosk', // ← add this
            status: 'new',                                   // ← add this (CRITICAL)
        ...
        // 跳转回首页
        window.location.href = 'thegoldenbowlsuccess.html';
    });
});